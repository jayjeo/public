...
reshape wide Fysit0 Fyst0 delta_Fyst, i(statefip date indb) j(yrim)
gen delta_Fsit=(Fysit02/Fyst02*delta_Fyst2)+(Fysit03/Fyst03*delta_Fyst3)+(Fysit04/Fyst04*delta_Fyst4)
replace delta_Fsit=0 if delta_Fsit==.
keep date statefip indb delta_Fsit
save delta_Fsit, replace 
...
**************************/
//!start
use delta_Fsit, clear 
merge m:1 statefip indb using Fsit0, nogenerate
rename Fsit684 Fsit0
merge m:1 statefip using Fst0, nogenerate
rename Fst684 Fst0
reshape wide delta_Fsit Fsit0 Fst0, i(statefip date) j(indb)
rename Fst01 temp_Fst0
drop Fst0*
rename temp_Fst0 Fst0

forvalues ind=1(1)7 {
    gen temp`ind'=Fsit0`ind'/Fst0*delta_Fsit`ind'
}
gen delta_Fst=temp1+temp2+temp3+temp4+temp5+temp6+temp7
keep statefip date delta_Fst
