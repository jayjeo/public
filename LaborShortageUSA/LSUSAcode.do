...
*********************************************/
ssc install asgen
net install xtabond2.pkg, from(http://fmwww.bc.edu/RePEc/bocode/x/)
...
/*********************************************
JOLTS Data (Seasonally adjusted)
*********************************************/
//!start
cd "${path}"
...
cd "${path}"
forvalues i=1(1)733{
    import delimited "https://raw.githubusercontent.com/jayjeo/public/master/LaborShortageUSA/csse_covid_19_daily_reports_us/`i'.csv", varnames(1) clear 
    gen filename=`i'
    save `i', replace 
}
use 1, clear
...
*********************************************/
cd "${path}"
import delimited "https://raw.githubusercontent.com/jayjeo/public/master/LaborShortageUSA/GDPbyState.csv", varnames(1) clear 
...
*********************************************/
//!start
***** Need to manually download dataset from https://www.dropbox.com/s/zaj7l6h7bdm6krl/cps_00006.dta
cd "E:\Dropbox\Study\UC Davis\Writings\Labor Shortage USA\data"
use cps_00006, clear
gen num=1
format num %12.0g
...
*********************************************/
cd "${path}"
use JOLTS_adjusted, clear
rename statemerge state
save JOLTS_adjusted_temp, replace 
...
keep if _merge==3
drop _merge
merge 1:1 date state using JOLTS_adjusted_temp
keep if _merge==3
drop _merge
merge 1:1 date state using gdp
keep if _merge==3
drop _merge
merge 1:1 date state using covid
keep if inlist(_merge,1,3)
drop _merge
replace covid=0.1 if covid==.
sort date state
tsfill 
drop if state==0
drop if state==15  // Hawaii
...
******* Manually decide p and q by states using the selection protocols provided by Borowczyk-Martins2013 
*** p selection protocol (estim_state `state') 
estim_state 56

*** q selection protocol (fig `state' `p')
fig 56 1

*** p, q selection results: "https://raw.githubusercontent.com/jayjeo/public/LaborShortageUSA/pq selection result.xlsx"

*** import p, q selection results
cd "${path}"
import delimited "https://raw.githubusercontent.com/jayjeo/public/master/LaborShortageUSA/pqselectionresult.csv", varnames(1) clear 
...
/*********************************************
Compare between OLS and Borowczyk-Martins2013 
*********************************************/
use k, clear
merge 1:1 state using pqselectionresult
keep if _merge==3
drop _merge
twoway (scatter k eta), xlabel(0(0.2)1) ylabel(0(0.2)1)
...
cd "${path}"
use master0, clear
merge m:1 state using pqselectionresult
keep if _merge==3 
drop _merge
sort state date
xtset state date
gen matcheff_unbiased=Hire/(u/100*l*(v/u)^eta) 
gen matcheff_biased=Hire/(u/100*l*(v/u)^k) 
keep if 719<=date  // 2019m12
*keep if 696<=date  // 2018m1
drop if date>=742  // 2021m11
drop if 723>=date  // 2020m04
sort state date
gen t=date-date[1]+1
format t %3.0f
...
gen lnmatcheff_unbiased=ln(matcheff_unbiased)
gen lnmatcheff_biased=ln(matcheff_biased)
gen lncovid=ln(covid)

gen D=0 if 723<=date
replace D=1 if 723>date
gen lninflowD=lninflow*D

**** FE regression
xi: xtreg v inflow gdp covid, fe vce(cluster state)
xi: xtreg v matcheff gdp covid, fe vce(cluster state)
xi: xtreg lnv lninflow lnmatcheff_unbiased gdp lncovid, fe vce(cluster state)
xi: xtreg lnv lninflow lnmatcheff_biased gdp lncovid, fe vce(cluster state)

**** Arellano and Bond (1991) First-differenced GMM estimator.
xtdpd v L.v inflow gdp, dgmm(L.v inflow gdp, lag(1))
xtdpd lnv L.lnv lninflow lnmatcheff_unbiased gdp lncovid, dgmm(L.lnv lninflow lnmatcheff_unbiased gdp lncovid, lag(1))

**** Arellano and Bond (1991) Linear dynamic panel GMM estimator.
xtabond v inflow, lags(2) pre(gdp covid)
...
xi: xtabond lnv lninflow lnmatcheff_unbiased I.t, lags(2) pre(gdp, lag(1,.)) pre(lncovid, lag(1,.)) twostep
xtabond2 lnv L.lnv L.(lninflow lnmatcheff_unbiased) gdp lncovid, robust twostep
xi: xtabond lnv lninflow lnmatcheff_unbiased I.t, lags(2) pre(gdp) pre(lncovid)
...
xtdpdml lnv lninflow lnmatcheff_unbiased, ylag(1 2) predetermined(gdp lncovid)
webuse abdata, clear

keep if state==8
tsset t
twoway(tsline v, yaxis(1))(tsline inflow, yaxis(2))(tsline matcheff_unbiased, yaxis(3))
